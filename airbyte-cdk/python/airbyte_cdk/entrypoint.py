...
class AirbyteEntrypoint(object):
    ...
    def handle_record_counts(message: AirbyteMessage, stream_message_count: DefaultDict[HashableStreamDescriptor, float]) -> AirbyteMessage:
        ...
        if message_type == Type.RECORD:
            ...
            stream_message_count[descriptor] += 1.0
        elif message_type == Type.STATE:
            stream_descriptor = message_utils.get_stream_descriptor(message)
            state = message.state
            state.sourceStats = state.sourceStats or AirbyteStateStats()
            state.sourceStats.recordCount = stream_message_count.get(stream_descriptor, 0.0)
