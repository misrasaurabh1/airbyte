...
class DefaultPaginator(Paginator):
    ...
    def _get_request_options(self, option_type: RequestOptionType) -> MutableMapping[str, Any]:
        ...
        if self.page_token_option and self._token is not None and isinstance(self.page_token_option, RequestOption):
            field_name = self.page_token_option.field_name.eval(config=self.config)
            if self.page_token_option.inject_into == option_type:
                options[field_name] = self._token  # type: ignore # field_name is always cast to an interpolated string

        if self.page_size_option and self.pagination_strategy.get_page_size():
            field_name = self.page_size_option.field_name.eval(config=self.config)
            if self.page_size_option.inject_into == option_type:
                options[field_name] = self.pagination_strategy.get_page_size()  # type: ignore # field_name is always cast to an interpolated string
