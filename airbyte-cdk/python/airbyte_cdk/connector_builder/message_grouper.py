...
class MessageGrouper:
    ...
    def _is_auxiliary_http_request(message: Optional[Dict[str, Any]]) -> bool:
        ...
        * Substream slice generation
        ...
        if not message:
            return False
        return MessageGrouper._is_http_log(message) and message.get("http", {}).get("is_auxiliary", False)
    ...
    def _process_field(self, field: Optional[Union[str, List[str], List[List[str]]]]) -> List[List[str]]:
        if not field:
            return [[]]
        if isinstance(field, str):
            return [[field]]
    ...
    def _pk_to_nested_and_composite_field(self, field: Optional[Union[str, List[str], List[List[str]]]]) -> List[List[str]]:
        return self._process_field(field)
    ...
    def get_message_groups(
        ...
        schema_inferrer = SchemaInferrer(self._process_field(stream.primary_key), self._process_field(stream.cursor_field))
        datetime_format_inferrer = DatetimeFormatInferrer()
    ...
    def _get_message_groups(
        ...
        latest_state_message = current_slice_descriptor = None
    ...
    def _is_page_http_request(json_message: Optional[Dict[str, Any]]) -> bool:
        return (
            ...
            and MessageGrouper._is_http_log(json_message)
