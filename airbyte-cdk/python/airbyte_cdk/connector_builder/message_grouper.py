...
class MessageGrouper:
    ...
    def _is_page_http_request(json_message: Optional[Dict[str, Any]]) -> bool:
        return json_message and (MessageGrouper._is_http_log(json_message) and not MessageGrouper._is_auxiliary_http_request(json_message))
    ...
    @staticmethod
    def _to_composite_field(field: Optional[Union[str, List[str], List[List[str]]]]) -> List[List[str]]:
        if not field:
            return [[]]
        if isinstance(field, str):
            return [[field]]
    ...
    @staticmethod
    def _to_nested_field(field: Union[str, List[str]]) -> List[List[str]]:
        if not field:
            return [[]]
        if isinstance(field, str):
            return [[field]]
    ...
    def get_message_groups(
        ...
        schema_inferrer = SchemaInferrer(
            self._to_composite_field(stream.primary_key),
            self._to_nested_field(stream.cursor_field),
        )
        datetime_format_inferrer = DatetimeFormatInferrer()
        ...
        for message_group in self._get_message_groups(
            self._read_stream(source, config, configured_catalog, state), schema_inferrer, datetime_format_inferrer, record_limit
        ):
            if isinstance(message_group, AirbyteLogMessage):
                log_messages.append(LogMessage(**{"message": message_group.message, "level": message_group.level.value}))
            elif isinstance(message_group, AirbyteTraceMessage) and message_group.type == TraceType.ERROR:
                log_messages.append(
                    LogMessage(**{"message": f"{message_group.error.message} - {message_group.error.stack_trace}", "level": "ERROR"})
                )
            elif isinstance(message_group, AirbyteControlMessage):
                if not latest_config_update or latest_config_update.emitted_at <= message_group.emitted_at:
                    latest_config_update = message_group
    ...
    def _is_auxiliary_http_request(message: Optional[Dict[str, Any]]) -> bool:
        ...
        * Substream slice generation
        """
        return message and MessageGrouper._is_http_log(message) and message.get("http", {}).get("is_auxiliary", False)
    ...
    def _get_message_groups(
        ...
        while records_count < limit and (message := next(messages, None)):
            json_object = self._parse_json(message.log) if message.type == MessageType.LOG else None
            json_message = json_object if isinstance(json_object, dict) else None
            if self._need_to_close_page(at_least_one_page_in_group, message, json_message):
                self._close_page(current_page_request, current_page_response, current_slice_pages, current_page_records)
                current_page_request = current_page_response = None
    ...
    def _read_stream(
        ...
        try:
            ...
            error_message = f"{e.args[0] if e.args else str(e)}"
            yield AirbyteTracedException.from_exception(e, message=error_message).as_airbyte_message()
    ...
    def _has_reached_limit(self, slices: List[StreamReadSlices]) -> bool:
        record_count = sum(len(page.records) for slice_ in slices for page in slice_.pages)
        return (
            len(slices) >= self._max_slices
            or any(len(slice_.pages) >= self._max_pages_per_slice for slice_ in slices)
            or record_count >= self._max_record_limit
        )
