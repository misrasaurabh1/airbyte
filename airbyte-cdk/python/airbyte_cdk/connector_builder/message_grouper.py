...
class MessageGrouper:
    ...
    def _is_page_http_request(json_message: Optional[Dict[str, Any]]) -> bool:
        return (
            bool(json_message) and MessageGrouper._is_http_log(json_message) and not MessageGrouper._is_auxiliary_http_request(json_message)
        )
    ...
    def _is_auxiliary_http_request(message: Optional[Dict[str, Any]]) -> bool:
        ...
        * Substream slice generation
        """
        return bool(message) and MessageGrouper._is_http_log(message) and message["http"].get("is_auxiliary", False)
    ...
    def get_message_groups(
        ...
        record_limit = min(record_limit or self._max_record_limit, self._max_record_limit)

        slices, log_messages, auxiliary_requests = [], [], []
        ...
        for message_group in self._get_message_groups(
            ...
            if isinstance(message_group, AirbyteLogMessage):
                log_messages.append(LogMessage(message=message_group.message, level=message_group.level.value))
            elif isinstance(message_group, AirbyteTraceMessage) and message_group.type == TraceType.ERROR:
                error_message = f"{message_group.error.message} - {message_group.error.stack_trace}"
                log_messages.append(LogMessage(message=error_message, level="ERROR"))
            elif isinstance(message_group, AirbyteControlMessage):
                if latest_config_update is None or latest_config_update.emitted_at <= message_group.emitted_at:
                    latest_config_update = message_group
            elif isinstance(message_group, AuxiliaryRequest):
                auxiliary_requests.append(message_group)
            elif isinstance(message_group, StreamReadSlices):
    ...
    def _get_message_groups(
        ...
        """
        records_count = 0
        at_least_one_page_in_group, current_page_records = False, []
        current_slice_descriptor, current_slice_pages = None, []
        current_page_request, current_page_response = None, None
        latest_state_message = None

        while records_count < limit and (message := next(messages, None)):
            json_object = self._parse_json(message.log) if message.type == MessageType.LOG else None
            if json_object and not isinstance(json_object, dict):
                raise ValueError(f"Expected log message to be a dict, got {json_object} of type {type(json_object)}")
            ...
            if (
                ...
                current_slice_descriptor = self._parse_slice_description(message.log.message)
                current_slice_pages, at_least_one_page_in_group = [], False
            elif message.type == MessageType.LOG and message.log.message.startswith(SliceLogger.SLICE_LOG_PREFIX):
                current_slice_descriptor = self._parse_slice_description(message.log.message)
            elif message.type == MessageType.LOG:
                if json_message and self._is_http_log(json_message):
                    if self._is_auxiliary_http_request(json_message):
                        yield AuxiliaryRequest(
                            title=(
                                "Parent stream: " + str(json_message["http"]["title"])
                                if json_message["airbyte_cdk"]["stream"]["is_substream"]
                                else str(json_message["http"]["title"])
                            ),
                            description=str(json_message["http"]["description"]),
                            request=self._create_request_from_log_message(json_message),
                            response=self._create_response_from_log_message(json_message),
                        )
                    else:
                    ...
                    yield message.log
            elif message.type == MessageType.TRACE and message.trace.type == TraceType.ERROR:
                yield message.trace
            elif message.type == MessageType.RECORD:
                current_page_records.append(message.record.data)
                records_count += 1
    ...
    def _close_page(
        ...
        """
        current_slice_pages.append(
            StreamReadPages(request=current_page_request, response=current_page_response, records=current_page_records.copy())
        )
        current_page_records.clear()
    ...
    def _read_stream(
        ...
        try:
            ...
            yield AirbyteTracedException.from_exception(e, message=f"{e.args[0] if e.args else str(e)}").as_airbyte_message()
    ...
    @staticmethod
    def _create_request_from_log_message(json_http_message: Dict[str, Any]) -> HttpRequest:
        request = json_http_message["http"]["request"]
        return HttpRequest(
            url=json_http_message["url"]["full"],
            http_method=request["method"],
            headers=request.get("headers"),
            body=request["body"].get("content", ""),
        )

    @staticmethod
    def _create_response_from_log_message(json_http_message: Dict[str, Any]) -> HttpResponse:
        response = json_http_message["http"]["response"]
        return HttpResponse(
            status=response["status_code"],
            body=response["body"].get("content", ""),
    ...
    def _has_reached_limit(self, slices: List[StreamReadSlices]) -> bool:
        ...
        record_count = sum(len(page.records) for _slice in slices for page in _slice.pages)
        return record_count >= self._max_record_limit
    ...
    @staticmethod
    def _clean_config(config: Dict[str, Any]) -> Dict[str, Any]:
        return {k: deepcopy(v) for k, v in config.items() if not k.startswith("__")}
